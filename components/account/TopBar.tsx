...
export default function TopBar({ user }: TopBarProps) {
  ...
  const [currentUser, setCurrentUser] = useState<CirglobUser>(user ?? DEFAULT_USER);
    ...
    <header className="fixed top-0 left-0 right-0 z-50 h-16 border-b border-white/10 bg-[#05060A]/70 backdrop-blur-xl">
      <div className="h-full max-w-7xl mx-auto px-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-white/70 tracking-tight">Cirglob</span>
        </div>
        ...
        <div className="relative">
          ...
          <IdentityDropdown open={open} onClose={() => setOpen(false)} />
        </div>
      </div>
    </header>
  );
